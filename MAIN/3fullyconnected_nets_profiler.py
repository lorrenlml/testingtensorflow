...
# TENSORS #
############
# Data to inject to the model: from dataframe to matrix.
...
def trainer(BATCH_SIZE, LEARNING_RATE, LOGDIR, OPT, MODEL, NN, EPOCHS, NAME, FORCE_CPU=False):
    if not os.path.isdir('logs&traces/'):
        os.mkdir('logs&traces/')
    ...
    with open(LOGDIR + "training.log", 'w') as _out:
        ...
        tf.summary.scalar("MSE",mse) #write a single scalar-valued tensor (NAME,TENSOR)
        mse_i = tf.losses.mean_squared_error(labels=input_labels, predictions=logits)
        tf.summary.scalar("MSE_i", mse_i)
        ...
        run_options = tf.RunOptions(trace_level=tf.RunOptions.FULL_TRACE)# Allow the full trace to be stored at run time.
        ...
        for e in range(EPOCHS):
            ...
            # Compute the mse of the most recent batch:
        ...
        mse_i, summ_i = session.run([mse_i,summary], feed_dict = fd_i, options=run_options, run_metadata=run_metadata)
        
        
        

        writer.add_summary(summ_i)
        ...
        return {
            ...
            'mse'       : mse,
            'inference_time' : 100*(inference_end_time - inference_start_time),
            #'mae'        : mae,
            #'rmse'       : rmse, 
...
######################################
# MAIN: GENERATE LOGS & METRICS#######
######################################

#############################################################################
global_results = dict()
n_1 = generador(hls, n_nets)
print('Nets for trainning and inference:' +  str(n_1))  
##############################################################################
    ...
    for opt in ['ADAM']:
        for n  in n_1:
            for device in ['CPU', 'GPU']:
                ...
                print ("Finished training and inference {} in {:.2f}s".format(NAME, time.time() - start))

#PARSE python dictionary and generate data for evaluation proccess:
fields = ['net', 'total_time', 'epoch_time', 'step_time', 'mse', 'inference_time', 'parameters', 'name', 'batch_size', 'learning_rate', 'opt', 'model', 'epochs']
